...
KISSY.add(function(S){
	//trim array.indexOf isArray
     ...
     filterByXType = function(items, xtype, shallow) {
         if (xtype === '*') {
             ...
             for (; i < length; i++) {
                 candidate = items[i];
                 if (candidate && candidate.isType && candidate.isType(xtype, shallow)) {
                     result.push(candidate);
                 }
             }
             return result;
     ...
     cq.Query = function(cfg){
    	 cfg = cfg || {};
         UFO.apply(this, cfg);
     }
     
     UFO.augment(cq.Query, {
         ...
         is: function(component) {
             var operations = this.operations,
                 components = S.isArray(component) ? component : [component],
             ...
             if (components.length === originalLength) {
                 if (operations.length > 1) {
                     for (i = 0, ln = components.length; i < ln; i++) {
                         if (S.indexOf(this.execute(), components[i]) === -1) {
                             return false;
                         }
                     }
                 }
	...
	var ComponentQuery = {
         ...
         * @param {UFO.container.Container} root The Container within which to perform the query.
         * If omitted, all Components within the document are included in the search.
         * 
         * This parameter may also be an array of Components to filter according to the selector.</p>
         * @returns {UFO.Component[]} The matched Components.
         * 
         * @member UFO.ComponentQuery
         */
         ...
         * @param {UFO.Component} component The Component to test
            ...
            while (selector && lastSelector !== selector) {
                ...
                if (tokenMatch) {
                    ...
                    // operation to the stack.
                        ...
                        operations.push({
                            method: filterByXType,
                            args: [S.trim(tokenMatch[2]), Boolean(tokenMatch[3])]
                        });
                    }
                ...
                while (!(modeMatch = selector.match(modeRe))) {
                    ...
                    for (i = 0; selector && i < length; i++) {
                        ...
                        if (i === (length - 1)) {
                            console.error('Invalid ComponentQuery selector: "' + arguments[0] + '"');
                        }
                    }
                }
